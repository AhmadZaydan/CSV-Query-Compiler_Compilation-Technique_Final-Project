...
from ast_nodes import (
    ...
    Expr, ColumnRef, Literal, Comparison, BooleanOp, SelectItem, 
    Star, ColumnSelect, NotOp, IsNullPredicate, InPredicate, 
    BetweenPredicate, LikePredicate, OffsetClause
)
...
class Parser:
    ...
    def parse_select_item(self) -> SelectItem:
        ...
        alias = None
        if self.match("AS"):
            alias_tok = self.expect("IDENT")
            alias = alias_tok.value
        return ColumnSelect(name=col_tok.value, alias=alias)
    ...
    def parse_bool_factor(self) -> Expr:
        ...
        return NotOp(expr) if negate else expr
    ...
    def parse_literal(self) -> Literal:
        ...
        Converts NUMBER to int/float, STRING stays as str.
        """
    ...
    def parse_predicate(self) -> Expr:
        col_tok = self.expect("IDENT")
        column = ColumnRef(name=col_tok.value)

        # IS [NOT] NULL
        if self.match("IS"):
            neg = self.match("NOT") is not None
            self.expect("NULL")
            return IsNullPredicate(column=column, negate=neg)
        ...
        if self.match("LIKE"):
            ...
            return LikePredicate(column=column, pattern=pat_tok.value)
