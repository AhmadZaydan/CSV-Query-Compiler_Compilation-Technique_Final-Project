...
def format_select(select_clause):
    parts = []
    for it in select_clause.items:
        if isinstance(it, Star):
            parts.append("*")
        else:
            if it.alias:
                parts.append(f"{it.name} AS {it.alias}")
            else:
                parts.append(it.name)
    prefix = "DISTINCT " if select_clause.distinct else ""
